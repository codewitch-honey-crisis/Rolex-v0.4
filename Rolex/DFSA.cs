...
namespace QUT.Gplex.Automaton
{
    ...
    internal class DFSA
    {
        ...
        public class DState : IComparable<DState>
        {
            ...
            internal string AbbreviatedStartConditionName
        ...
        public void Convert(NFSA nfa)
        {
            ...
            if (myTask.Verbose)
            {
                myTask.Msg.Write("Rolex: DFSA built");
            ...
            {
                myTask.ListDivider();
                myTask.ListStream.Write("Rolex: DFSA built. ");
        ...
        void WriteSummary(DateTime time)
        {
            ...
            myTask.ListDivider();
            myTask.ListStream.Write("Rolex: C# file emitted. ");
            myTask.ListStream.WriteLine(TaskState.ElapsedTime(time));

            myTask.ListDivider();
            myTask.ListStream.WriteLine("Rolex Summary");
            myTask.ListDivider();
                ...
                for (int i = 0; i <= maxAccept; i++)
                    if (stateList[i].needsBackup)
                    {
                        ...
                        myTask.ListStream.WriteLine(
                            ...
                            dSt.AbbreviatedStartConditionName, CharacterUtilities.Map(dSt.shortestStr), i, dSt.accept.Pattern);
                        myTask.ListStream.WriteLine(
        ...
        public void Minimize()
        {
            if (myTask.Minimize)
            {
                ...
                if (myTask.Verbose)
                {
                    myTask.Msg.Write("Rolex: DFSA minimized. ");
                    myTask.Msg.WriteLine(TaskState.ElapsedTime(start));
                }
                ...
                {
                    myTask.ListDivider();
                    myTask.ListStream.Write("Rolex: DFSA minimized. ");
                    myTask.ListStream.WriteLine(TaskState.ElapsedTime(start));
                }
            }
        }
        ...
        public void EmitScanner(TextReader sRdr, TextWriter sWrtr,List<LexRule> rules = null)
        {
            DateTime start = DateTime.Now;
            int frameVersion = 0;
            ...
            if (sRdr != null && sWrtr != null)
            {
                ...
                sWrtr.WriteLine( "//" );
                sWrtr.WriteLine( "//  This CSharp output file generated by Rolex" );
                sWrtr.WriteLine("// Copyright (C) by codewitch honey crisis, 2020.");
                sWrtr.WriteLine("// Rolex uses the GPLEX engine");
                sWrtr.WriteLine( "//  Gardens Point LEX (GPLEX) is Copyright (c) John Gough, QUT 2006-2014." );
                sWrtr.WriteLine( "//  Output produced by Rolex is the property of the user." );
                sWrtr.WriteLine( "//" );
                sWrtr.WriteLine( "//  Rolex Version:  " + myTask.VerString );
                ...
                sWrtr.WriteLine( "//  Rolex input file <" + myTask.InputInfo + ">" );
                sWrtr.WriteLine( "//  Rolex frame file <" + myTask.FrameName + ">" );
                sWrtr.WriteLine( "//" );
                ...
                sWrtr.WriteLine( "//  Option settings:{0}{1}{2}{3}{4}",
                    ...
                    (myTask.Shared ? " embedbuffers" : " noEmbedBuffers") );
                ...
                if (myTask.Verbose)
                {
                    ...
                    myTask.Msg.Write("Rolex: Shortest strings found. ");
                    myTask.Msg.WriteLine(TaskState.ElapsedTime(begin));
                    if (myTask.Summary)
                    {
                        myTask.ListDivider();
                        myTask.ListStream.Write("Rolex: Shortest strings found. ");
                    ...
                    if (line.StartsWith("##", StringComparison.Ordinal))
                    {
                        if (line.StartsWith("##-->", StringComparison.Ordinal))
                        {
                            ...
                            else if (selector == "usingDcl")
                            {
                                ...
                                sWrtr.WriteLine();
                                if (null != myTask.aast.nameString)
                                {
                                ...
                                if (myTask.Shared)
                                    TaskState.EmbedSharedCode(sWrtr);
                            ...
                            else if (selector == "constants")
                            {
                                EmitConstants(sWrtr, rules);
                            }
                ...
                if (myTask.Summary)
                {
                    if (backupCount > 0 && !foundShortestStrings)
                    {
                        ...
                        if (myTask.Verbose)
                        {
                            myTask.Msg.Write("Rolex: Reaching strings found. ");
                            myTask.Msg.WriteLine(TaskState.ElapsedTime(begin));
                        }
                        myTask.ListDivider();
                        myTask.ListStream.Write("Rolex: Reaching strings found. ");
                        myTask.ListStream.WriteLine(TaskState.ElapsedTime(begin));
                ...
                if (myTask.Verbose)
                {
                    myTask.Msg.Write("Rolex: C# file emitted. ");
                    myTask.Msg.WriteLine(TaskState.ElapsedTime(start));
                }
                sWrtr.Flush();
            }
        }
        internal void EmitConstants(TextWriter sWrtr,List<LexRule> rules)
        {
            if (null != rules)
            {
                foreach (var rule in rules)
                {
                    var o = rule.GetAttr("hidden", false);
                    if (o is bool && (bool)o) continue;
                    var s = rule.GetAttr("const", null) as string;
                    ...
                    sWrtr.Write("\t\t");
                    sWrtr.Write(myTask.aast.visibility);
                    sWrtr.Write(" const int ");
                    sWrtr.Write(_MakeSafeName(s));
                    sWrtr.Write(" = ");
                    sWrtr.Write(rule.Id.ToString());
                    sWrtr.WriteLine(";");

                }
            }
        }
        static HashSet<string> _keywords = _BuildKeywords();
        ...
        private string Translate(string originalText)
        {
            ...
            result = result.Replace("$Tokenizer", myTask.aast.tokenizerTypeName);
        ...
        internal void EmitActionCases(TextWriter sWrtr, int max)
        {
            ...
            for (int sOrd = eofNum + 1; sOrd <= max; sOrd++)
            {
                ...
                if (!emitted[sOrd])
                {
                    ...
                    // dying on newlines so nix it:
                    if (myTask.Verbose)
                    {
                        sWrtr.Write(" // ");
                        if (dSt.myDfaInst.StartConditionOrd != 0)
                            sWrtr.Write("In <{0}> ", dSt.AbbreviatedStartConditionName);
                        sWrtr.Write("Recognized {0}", dSt.accept.Pattern.Trim());
                             ...
                             SpansEqual(dSt.accept.aSpan, nSt.accept.aSpan))
                        {
                            ...
                            if (myTask.Verbose)
                            {
                                ...
                                if (nSt.myDfaInst.StartConditionOrd != 0)
                                    sWrtr.Write("In <{0}> ", nSt.AbbreviatedStartConditionName);
                                sWrtr.Write("Recognized {0}", nSt.accept.Pattern.Trim());
                                if (foundShortestStrings)
        ...
        {
            return l.buffer == r.buffer && l.startIndex == r.startIndex && l.endIndex == r.endIndex;
        }
        ...
        internal void EmitSlicedTables(TextWriter sWrtr)
        {
            ...
            if (hasLeftAnchors)
            {
                ...
                sWrtr.WriteLine("   static int[] anchorState = null;");
