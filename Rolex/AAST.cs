...
namespace QUT.Gplex.Parser {
    ...
    internal sealed class AAST {
        ...
        internal string scannerTypeName = "TokenizerEnumerator"; // Name of the generated scanner class.
        internal string tokenizerTypeName = "Tokenizer";         // Name of tokenizer class
        ...
        internal List<LexRule> lexRules;
        ...
        internal void SetScannerTypeName( string name ) {
            tokenizerTypeName= name;
            scannerTypeName = /*name +*/ "Enumerator";
            
        }
        internal void SetTokenizerTypeName(string name)
        {
        ...
        internal sealed class ReParser {
            ...
            internal RegExTree Parse(bool isRolexFmt=false) {
                try {
                    RegExTree tmp;
                    scan();
                    tmp = RegEx(isRolexFmt);
                    return tmp;
                }
            ...
            // RegEx : "<<EOF>>" | Expr ;
            internal RegExTree RegEx(bool isRolexFmt) {
                if (isRolexFmt)
                {
                    if('\''==chr)
                    {
                        scan();
                        RegExTree tmp = Expr(isRolexFmt);
                        if (chr != '\'')
                    ...
                    {
                        RegExTree tmp = LitString();
                        return tmp;
                    }
                    Error(104, index - 1, pat.Length - index + 1, null);
                    return null;
                }
                else
                {
            ...
            // Expr : ["^"] SimpleExpr ["$"] ;
            internal RegExTree Expr(bool isRolexFmt) {
                RegExTree tmp;
                if (!esc && chr == '^') {
                    scan();
                    tmp = new Unary( RegOp.leftAnchor, Simple(isRolexFmt) );
                }
                else
                    tmp = Simple(isRolexFmt);
                if (!esc && chr == '$') {
                    scan();
            ...
            // SimpleExpr : Term ["/" Term] ;
            internal RegExTree Simple(bool isRolexFmt) {
                RegExTree tmp = Term(isRolexFmt);
                if (!esc && chr == '/') {
                    scan();
                    return new Binary( RegOp.context, tmp, Term(isRolexFmt) );
                }
                return tmp;
            }

            // Term : Factor {"|" Factor} ;
            internal RegExTree Term(bool isRolexFmt) {
                RegExTree tmp = Factor(isRolexFmt);
                while (!esc && chr == '|') {
                    scan();
                    tmp = new Binary( RegOp.alt, tmp, Factor(isRolexFmt) );
                }
                return tmp;
            }

            // Factor : Primary {Primary} ;
            internal RegExTree Factor(bool isRolexFmt) {
                RegExTree tmp = Primary(isRolexFmt);
                while ((prStart[(int)chr] || esc) &&(!isRolexFmt || (esc || '\''!=chr)))
                    tmp = new Binary( RegOp.concat, tmp, Primary(isRolexFmt) );
                return tmp;
            }
            ...
            // Primary : (LitString | "(" Term ")" | Primitive) ["*" | "+" | Repetitions] ;
            internal RegExTree Primary(bool isRolexFmt) {
                RegExTree tmp;
                Unary pls;
                if (!isRolexFmt && !esc && chr == '"')
                    tmp = LitString();
                else if (!esc && chr == '(') {
                    scan();
                    tmp = Term(isRolexFmt);
    ...
    internal sealed class RuleDesc {
        ...
        internal void ParseRE( AAST aast,bool isRolexFmt = false ) {
            reAST = new AAST.ReParser( pattern, pSpan, aast ).Parse(isRolexFmt);
            SemanticCheck( aast );
        }
    ...
    internal sealed class PredicateLeaf : Leaf {
        ...
        internal void Populate( string name, AAST aast ) {
            ...
            if (aast.IsVerbose) {
                Console.WriteLine( "Rolex: Generating [:{0}:], {1}", name, Gplex.Automaton.TaskState.ElapsedTime( begin ) );
            }
        }
    }
