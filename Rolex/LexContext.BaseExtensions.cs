...
namespace LC
{
	partial class LexContext
	{
		...
		public bool TryReadWhiteSpace(bool readNewLines=true)
		{
			EnsureStarted();
			if (!readNewLines && '\n' == Current)
				return false;
			if (-1 == Current || !char.IsWhiteSpace((char)Current))
				return false;
			Capture();
			while (-1 != Advance() && (readNewLines || '\n' != Current) && char.IsWhiteSpace((char)Current)) Capture();
			return true;
		}
		/// <summary>
		...
		public bool TrySkipWhiteSpace(bool skipNewLines=true)
		{
			EnsureStarted();
			if (!skipNewLines && '\n' == Current)
				return false;
			if (-1 == Current || !char.IsWhiteSpace((char)Current))
				return false;
			while (-1 != Advance() && (skipNewLines || '\n'!=Current) && char.IsWhiteSpace((char)Current)) ;
			return true;
		}
		/// <summary>
