...
namespace LC
{
	partial class LexContext
	{
		...
		public bool TryReadCCommentsAndWhitespace(bool readNewLines=true)
		{
			bool result = false;
			while (-1 != Current)
			{
				if (!TryReadWhiteSpace(readNewLines) && !TryReadCComment())
					break;
				result = true;
			}
			if (TryReadWhiteSpace(readNewLines))
				result = true;
			return result;
		}
		/// <summary>
		...
		public bool TrySkipCCommentsAndWhiteSpace(bool skipNewLines=true)
		{
			bool result = false;
			while (-1 != Current)
			{
				if (!TrySkipWhiteSpace(skipNewLines) && !TrySkipCComment())
					break;
				result = true;
			}
			if (TrySkipWhiteSpace(skipNewLines))
				result = true;
			return result;
		}
		/// <summary>
